...
def convert_svg_to_png(svg_file, png_file):
    with open(svg_file, "rb") as f:
        svg_data = f.read()
    png_data = cairosvg.svg2png(bytestring=svg_data, output_width=300, output_height=315)
...
class ChessBoard(tk.Tk):
    ...
    def create_widgets(self, state: MyChess):
        ...
        # Text 
        self.moves_text = tk.Text(moves_frame, wrap="word", width=46, height=18, state="disabled")
        self.moves_text.grid(row=1, column=0, sticky="nw")
        self.moves_text.config(cursor="")
        ...
        # User input box
        self.user_input = tk.Text(user_frame, wrap="word", width=46, height=18)
        ...
        self.resp_text = tk.Text(resp_frame, width=46, height=22, wrap="word")
        self.resp_text.grid(row=1, column = 0, sticky="nw")
        self.resp_text.config(cursor="")
        self.resp_text.bind("<Button-1>", self.disable_click)
